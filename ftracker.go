...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	var calories float64
	meanSpeedInMeters := (meanSpeed(action, duration) * kmhInMsec)
	if duration == 0 {
		return 0
	}
	calories = ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInMeters, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
